...
public:
  ...
  {
    //to publish pcl_scan message
    pub_ = n_.advertise<geometry_msgs::Twist>("mod_cmd_vel",1000);

    //to subscribe to cmd_vel messages
    sub_ = n_.subscribe("/slam_cloud",1000, &SubscribeAndPublish::callback, this);
  }

  void callback(const sensor_msgs::PointCloud2 &cloud_msg)// callback function for subscriber
  {
