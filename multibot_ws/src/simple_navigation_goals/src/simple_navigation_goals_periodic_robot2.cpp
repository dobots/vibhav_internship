...
int main(int argc, char** argv){
  ...
  ROS_INFO("Sending goal to robot2");
  ac.sendGoal(goal);
  ...
  if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
    ROS_INFO("Hooray, the robot2 moved 1 meter forward in 7 seconds");
  else{
    count++;
    ROS_INFO("robot2 failed to move forward 1 meter in 7 seconds for some reason");
    }
  }
  return 0;
}
