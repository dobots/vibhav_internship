...
class robot:
    ...
    def __init__(self, name):
        self.assigned_point = []
        self.name = name
        #rospy.loginfo('Robot function: I am here')     #-works
        self.global_frame = rospy.get_param('~global_frame', 'map')         #Initially /map
        self.robot_frame = rospy.get_param('~robot_frame', 'base_link')
        self.plan_service = rospy.get_param(
            '~plan_service', '/move_base/GlobalPlanner/make_plan')              # Initially '/move_base_node/NavfnROS/make_plan'
        self.listener = tf.TransformListener()
        self.listener.waitForTransform(
            self.global_frame, self.name+'/'+self.robot_frame, rospy.Time(0), rospy.Duration(10.0))
        cond = 0
        self.goal_counter = 0            #Added
        while cond == 0:
            try:
        ...
        self.client = actionlib.SimpleActionClient(self.name+'/move_base', MoveBaseAction)        #self.name+'/move_base' originally
        self.client.wait_for_server()
        #rospy.loginfo(self.client)
        #rospy.loginfo('I am here')      #for debug
    ...
    def getPosition(self):
        ...
        while cond == 0:
            try:
                ...
                cond == 0
        self.position = array([trans[0], trans[1]])
        #rospy.loginfo('getPosition: I am here')      #for debug
