...
#from PointArray.msg import points
import tf
...
from functions_algo_edit import robot,informationGain,discount
...
def callBack(data):
	global frontiers
	#rospy.loginfo("Assigner: I am here")
	frontiers=[]
...
def node():
	...
	frontiers_topic= rospy.get_param('~frontiers_topic','filtered_points')	#Initially filtered_points
...
#wait if map is not received yet
	...
	#rospy.loginfo('Assigner: I am here')
	#rospy.loginfo(len(namespace))

	if len(namespace)>0:
		for i in range(0,n_robots):
			robots.append(robot(namespace+str(i+namespace_init_count)))
	elif len(namespace)==0:
			robots.append(robot(namespace))			# This line was not working as within the robot class, then wait service was still happening
			#rospy.loginfo('Assigner: I am here')
			#rospy.loginfo(namespace)
			#rospy.loginfo(len(namespace))
			#rospy.loginfo(len(robots))
